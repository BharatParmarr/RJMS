...
const Dashboard = ({ data }: any) => {
    ...
    const [itemsSold, setItemsSold] = React.useState(data.items_sold);
    const [itemsrevenue, setItemsRevenue] = React.useState(data.item_revenue);
    const [order_time, setOrderTime] = React.useState(data.order_time);
    const [category_data, setCategoryData] = React.useState(data.category_data);
    const [item_creation_time, setItemCreationTime] = React.useState(data.item_creation_time);
    ...
    useEffect(() => {
        ...
        fetch(API_HOST + '/api/AnalysisMonth?month=' + month + '&year=' + year + '&restorant_id=' + id, {
            method: 'GET',
            headers: {
            ...
            .then(data => {
                ...
                if (data.item_revenue.length > 0) {
                    setItemsRevenue(data.item_revenue);
                } else {
                    setItemsRevenue([{}]);
                }
                setOrderTime(data.order_time);
                setCategoryData(data.category_data);
                if (data.item_creation_time.length > 0) {
                    setItemCreationTime(data.item_creation_time);
                } else {
                    setItemCreationTime([{}]);
                }
                console.log('Success:', month, year);
            })
            .catch((error) => {
