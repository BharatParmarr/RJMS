...
function Hostel_info_change({ hosteldata }: any) {
    ...
    const handleSubmit = () => {
        if (!hostelDetails.name || !hostelDetails.address || !hostelDetails.phone || !hostelDetails.email || !hostelDetails.website) {
        ...
        if (id) { formData.append('service_shop_id', id); } else {
        ...
        api.put(``, formData).then((response) => {
...
function Settings_component() {
    ...
    useEffect(() => {
        api.get(`?service_shop_id=${id}`).then((response) => {
            if (response.status === 200) {
    ...
    useEffect(() => {
        api.get(`/Staf?service_shop_id=` + id).then((response) => {
    ...
    const handleSubmit = (e: any) => {
        ...
        api.put(`/Manager`, data).then((response) => {
    ...
    const removeManger = () => {
        api.delete(`/Manager?service_shop_id=${id}&manager=1&manager_name=${Manager}`).then((response) => {
            if (response.status === 200) {
                setManager('')
            }
        }).catch((error) => {
    ...
    const removeStaff = (staff: any) => {
        api.delete(`/Staff?service_shop_id=${id}&manager=0&manager_name=${staff}`).then((response) => {
            if (response.status === 200) {
    ...
    const handleSubmit2 = (e: any) => {
        ...
        api.post(`Staf`, { service_shop: id, username: Staff_name }).then((response) => {
            if (response.status === 200) {
...
function Settings_restorant({ For = "Shop", Settings = Settings_component }: SettingsProps) {
    const { theme } = useTheme()
